...
import { currentUserPermissions, Permission, User, currentUser } from '../../../core/auth';
...
export class BaseComponent implements OnInit, OnDestroy {
	...
	loadRolesWithPermissions() {
		...
		res.forEach((pm: Permission) => this.permissionsService.addPermission(pm.tag));
		});
		this.unsubscribe.push(subscr);
	}
	...
	loadMenu()
	{
		...
		this.user$.subscribe(pf => {	
			if (pf)
			{
				this.menuConfigService.loadConfigs({ header: pf.menu, aside: pf.menu });		
			}
			else
			{
				this.menuConfigService.clear();
			}

			if(this.menuConfigService.getMenus().header)
			this.itensMenu = this.menuConfigService.getMenus().header.items;
		});	
	}
}
