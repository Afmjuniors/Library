...
export interface RolesState extends EntityState<UserProfile> {
...
export const adapter: EntityAdapter<UserProfile> = createEntityAdapter<UserProfile>({
    selectId: UserProfile => UserProfile.profileUserId
  });
...
export function rolesReducer(state = initialRolesState, action: RoleActions): RolesState {
    switch  (action.type) {
        ...
        case RoleActionTypes.RoleCreated: return adapter.addOne(action.payload.role, {
            ...state, lastCreatedRoleId: action.payload.role['userProfileId']
        });
