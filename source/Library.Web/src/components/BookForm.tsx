...
export const BookForm: React.FC<BookFormProps> = ({
  ...
  const handleSave = () => {
    // Validação
    const nameValidation = validation.book.name(formData.name);
    if (!nameValidation.isValid) {
      Alert.alert('Erro', nameValidation.message);
      return;
    }

    const authorValidation = validation.book.author(formData.author);
    if (!authorValidation.isValid) {
      Alert.alert('Erro', authorValidation.message);
      return;
    }
