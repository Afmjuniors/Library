...
export const fetchBooks = createAsyncThunk(
  ...
  async ({ params, page = 1 }: { params?: SearchBookParams; page?: number }, { rejectWithValue }) => {
    try {
      const response = await apiService.books.getBooks();
      return response;
    } catch (error: any) {
...
export const fetchBook = createAsyncThunk(
  ...
  async (bookId: number, { rejectWithValue }) => {
    try {
      const book = await apiService.books.getBooks(); // Temporário - implementar getBook
      return book;
    } catch (error: any) {
...
export const createBook = createAsyncThunk(
  ...
  async (bookData: Partial<Book>, { rejectWithValue }) => {
    try {
      const book = await apiService.books.addBook(bookData);
      return book;
    } catch (error: any) {
...
export const updateBook = createAsyncThunk(
  ...
  async ({ bookId, bookData }: { bookId: number; bookData: Partial<Book> }, { rejectWithValue }) => {
    try {
      const book = await apiService.books.updateBook(bookId, bookData);
      return book;
    } catch (error: any) {
