...
import { SignApproval } from '../_models/signAprovval.model';
...
export class SignatureService {
    ...
    GetSignatureObject(signature: string): Observable<SignApproval> {
        const userToken = localStorage.getItem(environment.authTokenKey);
        let header = new HttpHeaders({ 'Authorization': 'Bearer ' + userToken });
        header.append('Content-Type', 'application/json');
        return this.http.get<SignApproval>(URL_BASE + '/AccessControl/ReadSignature?signature=' + signature, { headers: header });
    }
